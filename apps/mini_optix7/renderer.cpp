...
MainRenderer::createRaygenPrograms()
{
  ...
  char log[2048];
  size_t sizeof_log = sizeof(log);
  OPTIX_CHECK(optixProgramGroupCreate(optix_context, &pgDesc, 1, &pgOptions, log, &sizeof_log, &raygenPGs[0]));
  general_log_cb(log, sizeof_log);
}
...
MainRenderer::buildSBT()
{
  ...
  raygenRecordsBuffer.alloc_and_upload_async(raygenRecords, stream);
